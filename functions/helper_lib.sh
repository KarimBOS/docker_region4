...
# Devuelve la ruta absoluta de una cadena dada
abspath () { case "$1" in /*)printf "%s\n" "$1";; *)printf "%s\n" "$PWD/$1";; esac; }

# Audit rules default path
# Ruta por defecto de las reglas de auditoría
auditrules="/etc/audit/audit.rules"

# Check for required program(s)
# Verifica si los programas requeridos están instalados
req_programs() {
  for p in $1; do
    command -v "$p" >/dev/null 2>&1 || { printf "Required program not found: %s\n" "$p"; exit 1; }
    # Verifica si el programa está instalado, si no, muestra el error
  done
...
# Compara versiones de software en el formato X.Y.Z
do_version_check() {
  [ "$1" = "$2" ] && return 10
...
# Extrae los argumentos de la línea de comandos de los procesos en ejecución más recientes con el mismo nombre que el primer parámetro
...
# Extrae los argumentos de la línea de comandos acumulativos para el demonio Docker
#
# Si se especifica varias veces, se devuelven todas las coincidencias.
# Tiene en cuenta las variantes largas y cortas, se llama con la opción corta.
# No tiene en cuenta los valores predeterminados o implícitos de las opciones.
get_docker_cumulative_command_line_args() {
  ...
  # normaliza las opciones largas conocidas a sus versiones cortas
  sed \
    -e 's/\-\-debug/-D/g' \
    -e 's/\-\-host/-H/g' \
    -e 's/\-\-log-level/-l/g' \
    ...
    # normaliza los parámetros separados por espacios a -O=VALOR
    sed \
      -e 's/\-\([DHlv]\)[= ]\([^- ][^ ]\)/-\1=\2/g' \
      |
    # get the last interesting option
    # obtiene la última opción interesante
    tr ' ' "\n" |
    grep "^${OPTION}" |
    # normalize quoting of values
    # normaliza las comillas de los valores
    sed \
      -e 's/"//g' \
      -e "s/'//g"
}
...
# Extrae los argumentos efectivos de la línea de comandos para el demonio Docker
#
# Tiene en cuenta las especificaciones múltiples, toma la última opción.
# Tiene en cuenta las variantes largas y cortas, se llama con la opción corta.
# No tiene en cuenta los valores predeterminados o implícitos de las opciones.
