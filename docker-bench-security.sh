...
save_reports() {
  ...
  if [ -f "$logger" ]; then
    ...
    mv "$txt_report" "$download_dir/"
    echo "Informe TXT guardado como $download_dir/$txt_report"
  else
    echo "Error: No se generó el archivo de registro ($logger). Revisa la ejecución del programa."
  fi
